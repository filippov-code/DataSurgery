...
- Функция шифрования сообщения с помощью AES + FPE
...
Вместе с AES используется FPE (format-preserving encryption). FPE используется для шифрования блока DSMD. Тогда размер метаданных не изменяется и DSMD по-прежнему занимает 9 байт в скрываемом сообщении.
